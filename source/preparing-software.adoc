...
Программы, написанные на языках сценариев оболочки, таких как  _bash_, интерпретируются в необработанном виде.
...
В следующем примере мы создаем исправление из исходного исходного кода с помощью 
``diff``, а затем используем ``patch``. Исправление использует в следующих разделяах при создании RPM и работе со .spec-файлом. xref:working-with-spec-files[].
...
. Сохраним оригинальный исходный код:
+
[source,bash]
----
